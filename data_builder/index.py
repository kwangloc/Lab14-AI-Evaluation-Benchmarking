...
CHUNKS_OUTPUT_FILE = Path(__file__).parent / "chunks.jsonl"
...
def build_index(docs_dir: Path = DOCS_DIR, db_dir: Path = CHROMA_DB_DIR, chunks_output: Path = CHUNKS_OUTPUT_FILE) -> None:
    """
    ...
    client = chromadb.PersistentClient(path=str(db_dir))
    collection = client.get_or_create_collection(
        name=os.getenv("COLLECTION_NAME", "rag_lab"),
        metadata={"hnsw:space": "cosine"},
    )
    ...
    print(f"Chunks đã lưu vào: {chunks_output}")
